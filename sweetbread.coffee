...
arrow = blue " → "
global.time = ()-> new Date().toLocaleTimeString "en-US", hour12: false
global.log = (...msgs)-> console.log yellow(time()) + arrow + msgs.join ", "
...
global.rm = (pattern)->
  ...
  log "Deleted #{pattern} " + blue "(#{Math.ceil performance.now() - start}ms)"
...
Compilers.html = (paths, dest)->
  ...
  fs.writeFileSync dest, contents.join "\n\n"
  log "Compiled #{dest} " + blue "(#{Math.ceil performance.now() - start}ms)"

Compilers.kit = (path, dest)-> # Note — just 1 file at a time
  start = performance.now()
  compiled = kit path
  compressed = htmlmin.minify compiled,
...
Compilers.static = (path, dest)-> # Note — just 1 file at a time
  start = performance.now()
